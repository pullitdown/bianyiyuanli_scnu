...
int synxax_table[100][10]=
{
1,2,3,4,5,612,31,21,321,23,
...
int main()
{
    ...
    {
        cout << i << "\t"
             << "values:" << vstrs[i].first << "\ttype:" << type_of_str[pointin(vstrs[i].first, vstrs[i].second, 1) - 1] << endl;
    }
    vstrs.clear(); //重置状态表
    ...
    {
        cout << i << "\t"
             << "values:" << vstrs[i].first << "\ttype:" << type_of_str[pointin(vstrs[i].first, vstrs[i].second, 0) - 1] << endl;
    }
    system("pause");
}